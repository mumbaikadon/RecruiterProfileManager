/**
 * Standalone test module for the gap analysis functionality
 */
...
// Resume text extracted from Drew Corrigan's CV
const resumeText = `
Drew Corrigan
...
/**
 * The main function to run the gap analysis
 */
export async function testAnalyze() {
  try {
    ...
    console.log(`Overall Match Score: ${result.overallScore}%`);
    console.log(`Confidence: ${result.confidenceScore * 100}%`);
    
    console.log("\n==== STRENGTHS ====");
    result.relevantExperience.forEach((exp, i) => {
      console.log(`${i+1}. ${exp}`);
    });
    
    console.log("\n==== WEAKNESSES ====");
    result.skillsGapAnalysis.missingSkills.forEach((skill, i) => {
      console.log(`${i+1}. ${skill}`);
    });
    
    console.log("\n==== DETAILED GAP ANALYSIS ====");
    if (result.skillsGapAnalysis.gapDetails && result.skillsGapAnalysis.gapDetails.length > 0) {
      result.skillsGapAnalysis.gapDetails.forEach((gap, i) => {
        console.log(`\nGAP CATEGORY ${i+1}: ${gap.category} (${gap.importance})`);
        console.log(`Impact: ${gap.impact}`);
        
        console.log("Specific Gaps:");
        gap.gaps.forEach((g, j) => {
          console.log(`  ${j+1}. ${g}`);
        });
        
        console.log("Suggestions:");
        gap.suggestions.forEach((s, j) => {
          console.log(`  ${j+1}. ${s}`);
        });
      });
    } else {
      console.log("No detailed gap information available");
    }
