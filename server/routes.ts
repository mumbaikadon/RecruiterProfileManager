  ...
  app.put("/api/jobs/:id/status", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      if (
        ...
        // Process each submission and delete resume files
        ...
        for (const submission of submissions) {
          ...
          if (resumeData?.fileName) {
            ...
            try {
              ...
              console.log(`Deleted resume file: ${resumeData.fileName}`);
              deletedFiles++;
            } catch (fileError) {
              console.warn(`Resume file already deleted or not found: ${resumeData.fileName}`);
            }
          }
        }
        
        console.log(`Job closure cleanup completed. Deleted ${deletedFiles} resume files.`);
        
        // Update the activity log with cleanup results
        await storage.createActivity({
          type: "system_cleanup",
          jobId: id,
          message: `Cleanup completed for closed job ${job.jobId}. Deleted ${deletedFiles} resume files while preserving candidate data.`,
        });
      }
