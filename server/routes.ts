...
`;
  ...
  // Phase 1: Fast Upload Endpoint - Quick file storage without processing
  app.post("/api/profile-resumes/fast-upload", requireAuth, bulkUpload.array('resumes', 20), async (req: Request, res: Response) => {
    const startTime = Date.now();
    const { profileLogger } = await import('./logger');
    
    try {
      profileLogger.apiRequest('POST', '/api/profile-resumes/fast-upload', (req as any).user?.id);
      
      if (!req.files || !Array.isArray(req.files) || req.files.length === 0) {
        profileLogger.apiError('POST', '/api/profile-resumes/fast-upload', 'No files uploaded', 400);
        return res.status(400).json({ message: "No files uploaded" });
      }
      ...
      console.log(`Starting fast upload of ${files.length} files`);
      
      // Create upload batch
      const batch = await storage.createUploadBatch(files.length, (req as any).user.id);
      ...
      // Process files without text extraction - just save to temp table
      for (const file of files) {
        try {
          ...
          // Save file to filesystem immediately
          const { saveFile } = await import('./file-utils');
          const filePath = await saveFile(file.buffer, file.originalname);
          
          // Store in temp table
          const fileType = file.mimetype === 'application/pdf' ? 'pdf' : 'docx';
          const tempUpload = await storage.createTempProfileUpload({
            batchId: batch.batchId,
            filename: file.originalname,
            fileType,
            fileSize: file.size,
      ...
      await storage.updateUploadBatchProgress(batch.batchId, results.successful.length, results.failed.length);
      
      // Create activity for fast upload
      await storage.createActivity({
        type: "system_integration",
        userId: (req as any).user.id,
        message: `Fast upload initiated: ${results.successful.length} files uploaded, ${results.failed.length} failed (Batch: ${batch.batchId})`
      });

      const duration = Date.now() - startTime;
      console.log(`Fast upload completed in ${duration}ms: ${results.successful.length}/${results.total} successful`);
      ...
      const duration = Date.now() - startTime;
      const errorMsg = (error as Error).message;
      profileLogger.apiError('POST', '/api/profile-resumes/fast-upload', errorMsg, 500);
      res.status(500).json({ message: errorMsg });
    }
  });
  ...
  // Status tracking endpoints for three-phase upload system
  
  // Get batch status by batch ID
  app.get("/api/profile-resumes/batch/:batchId/status", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      // Get temp uploads for this batch
      const { getTempUploadsByBatch } = await import('./storage-helpers');
      const tempUploads = await getTempUploadsByBatch(batchId);
      
      const statusCounts = {
        pending: tempUploads.filter(u => u.uploadStatus === 'pending').length,
        processing: tempUploads.filter(u => u.uploadStatus === 'processing').length,
        processed: tempUploads.filter(u => u.uploadStatus === 'processed').length,
        failed: tempUploads.filter(u => u.uploadStatus === 'failed').length
      };

      res.json({
        batch,
        statusCounts,
        uploads: tempUploads.map(upload => ({
          id: upload.id,
          filename: upload.filename,
          status: upload.uploadStatus,
          error: upload.errorMessage,
          attempts: upload.processingAttempts,
  ...
  app.get("/api/profile-resumes/processor/status", requireAuth, async (req: Request, res: Response) => {
    try {
      const { getProcessorStatus } = await import('./background-processor');
      const status = getProcessorStatus();
      
      // Get counts of pending uploads
      const pendingUploads = await storage.getPendingTempUploads(50);
  ...
  app.post("/api/profile-resumes/processor/process", requireAuth, async (req: Request, res: Response) => {
    try {
      const { processNow } = await import('./background-processor');
      
      // Trigger processing in background (don't wait)
      processNow().catch(error => {
  ...
  app.post("/api/profile-resumes/processor/cleanup", requireAuth, async (req: Request, res: Response) => {
    try {
      const { cleanupNow } = await import('./background-processor');
      
      // Trigger cleanup in background (don't wait)
      cleanupNow().catch(error => {
        console.error('Manual cleanup error:', error);
      });
      
      res.json({ message: "Background cleanup triggered" });
    } catch (error) {
      res.status(500).json({ message: (error as Error).message });
    }
  });

  // Get pending uploads summary (for admin monitoring)
  app.get("/api/profile-resumes/pending", requireAuth, async (req: Request, res: Response) => {
    try {
      const limit = parseInt(req.query.limit as string) || 20;
      const pendingUploads = await storage.getPendingTempUploads(limit);
